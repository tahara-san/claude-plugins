...
6. If at least one finding IS BLOCKING (split-verdict rounds go through item 7's dispute cycle FIRST — only upheld or re-affirmed blockers enter this step): patch **only** what closes those blockers (remedy-scope binding — `review-policy` §5), then resubmit that document to BOTH lanes. Increment the round count; on reaching **4** document-mutating rounds, enter convergence mode and escalate to the user after one more bounded blocker round (the 5th).
7. If the round ends with **split verdicts** — one lane `PASS`, the other `CHANGES_REQUIRED` — do not override the failing lane and do not escalate: run the `review-policy` §7a cross-lane dispute cycle. The passing lane meta-reviews the disputed blockers (Codex: `/codex-chunk dispute --role meta`; Fable: SendMessage to that document's reviewer, or a fresh subagent given the prior-verdict summary). If every blocker draws an objection, the failing lane reconsiders on the same bytes (Codex: `/codex-chunk dispute --role reconsider`; Fable: SendMessage). A final `PASS` closes the dispute (`reject-unsupported`); a re-affirmed `CHANGES_REQUIRED` stands — patch only those blockers and resubmit per step 6. If both lanes fail and you judge a finding non-blocking, use the §7 clarification-rerun flow; escalation comes only from the round budget.
