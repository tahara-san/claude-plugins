...
Step 7's second lane — the **Claude Code Opus 4.8 (xhigh reasoning) Review**
...
`model: "opus"`; the prompt must request xhigh-reasoning thoroughness since
the Agent tool has no effort parameter — if the harness exposes an effort
selector, set it to `xhigh`; if the harness no longer offers `"opus"`, use
...
interchangeable: the Opus 4.8 lane never replaces `/codex-chunk`, and
`/codex-chunk` never replaces the Opus 4.8 lane.

The plan documents this skill writes also reference `/simplify` (built into
Claude Code itself), `/codex-chunk`, and the Claude Code Opus 4.8 (xhigh
...
3. **MANDATORY:** Run a parallel review round on all changed files in the phase — Opus 4.8 review in background + `/codex-chunk` (PREREQUISITE: step 2 must be complete). Round 1 is full-coverage; record the reviewed state when clean.
...
7. **MANDATORY:** Run a parallel review round on ALL changed files together — Opus 4.8 review in background + `/codex-chunk` (PREREQUISITE: step 6 must be complete). The first holistic round is full-coverage; record the reviewed state when clean.
...
12. If the build fails:
    ...
    c. **MANDATORY:** Run a review round on the fix changes at the scope the delta-round rules require (Opus 4.8 in background + `/codex-chunk` for code fixes); iterate until the required lanes are clean
...
- [ ] Per-phase: `/simplify` → parallel review round (`/codex-chunk` ∥ Opus 4.8 review) passes — no CRITICAL or worth-addressing WARNINGs from either lane; re-rounds at delta-round-rule scope with recorded baselines
- [ ] Holistic `/simplify` → parallel review round (`/codex-chunk` ∥ Opus 4.8 review) passes (skip if single-phase)
...
- **Orchestrator**: Claude Opus 4.8, xhigh effort
...
- [ ] Per-phase: `/simplify` → parallel review round (`/codex-chunk` ∥ Opus 4.8 review) passes — no CRITICAL or worth-addressing WARNINGs from either lane; re-rounds at delta-round-rule scope with recorded baselines
- [ ] Holistic `/simplify` → parallel review round (`/codex-chunk` ∥ Opus 4.8 review) passes across all phases
- [ ] `npm run build` passes
...
- [ ] `/simplify` → parallel review round (`/codex-chunk` ∥ Opus 4.8 review) passes (no CRITICAL or worth-addressing WARNINGs from either lane)
- [ ] Mark phase N complete in `progress.md`
```

---

### Step 7: Parallel Plan Review — Codex ∥ Opus 4.8 (Finalization)
...
1. Launch the Claude Code Opus 4.8 (xhigh reasoning) Review in the background: a `general-purpose` subagent via the Agent tool with `run_in_background: true` and `model: "opus"` (if the harness no longer offers `"opus"`, use the newest flagship selector it does offer and tell the user — flag the substitution prominently, never substitute silently). The prompt must request xhigh-reasoning thoroughness (no effort parameter exists on the Agent tool; if an effort selector exists, use `xhigh`), name the document, give review context, the CRITICAL/WARNING/INFO taxonomy, and the `[SEVERITY] file:line — description` output format (same as `/codex-chunk`, so the two lanes' findings merge cleanly).
2. Submit the same document to `/codex-chunk` for review while the Opus 4.8 lane runs
3. Collect BOTH results (wait for the background Opus 4.8 notification — never judge on the Codex result alone) and merge the findings
...
6. Confirmation that all documents passed BOTH review lanes (`/codex-chunk` and the Claude Code Opus 4.8 (xhigh reasoning) Review), with round counts per document
